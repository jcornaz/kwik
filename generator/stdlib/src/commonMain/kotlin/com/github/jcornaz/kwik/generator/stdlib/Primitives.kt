 ...
 */
public fun Generator.Companion.ints(min: Int = Int.MIN_VALUE, max: Int = Int.MAX_VALUE): Generator<Int> {
    ...
    return Generator { it.nextInt(range) }.withSamples(samples, probability = 0.3)
}
 ...
 */
public fun Generator.Companion.positiveInts(max: Int = Int.MAX_VALUE): Generator<Int> = ints(0, max)
 ...
 */
public fun Generator.Companion.negativeInts(min: Int = Int.MIN_VALUE): Generator<Int> = ints(min, -1)
 ...
 */
public fun Generator.Companion.naturalInts(max: Int = Int.MAX_VALUE): Generator<Int> = ints(1, max)
 ...
 */
public fun Generator.Companion.nonZeroInts(min: Int = Int.MIN_VALUE, max: Int = Int.MAX_VALUE): Generator<Int> =
 ...
 * Contains the samples: -1, 0, 1, [min] and [max]
 */
public fun Generator.Companion.longs(min: Long = Long.MIN_VALUE, max: Long = Long.MAX_VALUE): Generator<Long> {
    require(max >= min) { "Max must be greater than min but min was $min and max was $max" }

    val range = min..max
    val samples = listOf(0, 1, -1, min, max).filter { it in range }

    return Generator { it.nextLong(range) }.withSamples(samples, probability = 0.3)
}
 ...
 * Contains the samples: 0, 1 and [max]
 */
public fun Generator.Companion.positiveLongs(max: Long = Long.MAX_VALUE): Generator<Long> = longs(0L, max)
 ...
 * Contains the samples: -1 and [min]
 */
public fun Generator.Companion.negativeLongs(min: Long = Long.MIN_VALUE): Generator<Long> = longs(min, -1L)
 ...
 * Contains the samples: 1 and [max]
 */
public fun Generator.Companion.naturalLongs(max: Long = Long.MAX_VALUE): Generator<Long> = longs(1L, max)
 ...
 * Contains the samples: -1, 1, [min] and [max]
 */
public fun Generator.Companion.nonZeroLongs(min: Long = Long.MIN_VALUE, max: Long = Long.MAX_VALUE): Generator<Long> =
 ...
 */
public fun Generator.Companion.floats(
 ...
 */
public fun Generator.Companion.positiveFloats(max: Float = Float.MAX_VALUE): Generator<Float> =
    floats(0f, max)
 ...
 */
public fun Generator.Companion.negativeFloats(min: Float = -Float.MAX_VALUE): Generator<Float> =
...
public fun Generator.Companion.doubles(
    ...
    val generators = mutableListOf(
        0.3 to Generator { it.nextDouble(min, until) },
        0.2 to Generator { it.nextDouble(-100.0, 100.0).coerceIn(min, max) },
        0.1 to of(min, max)
    )
    ...
    if (min <= 1.0 && max >= 0.0)
        generators += 0.1 to Generator { it.nextDouble().coerceIn(min, max) }
 ...
 * Contains the samples: 0.0, 1.0 and [max]
 */
public fun Generator.Companion.positiveDoubles(max: Double = Double.MAX_VALUE): Generator<Double> =
    doubles(0.0, max)
 ...
 * Contains the samples: -1.0 and [min]
 */
public fun Generator.Companion.negativeDoubles(min: Double = -Double.MAX_VALUE): Generator<Double> =
 ...
 * Contains the samples: -1, 1, [min] and [max]
 */
public fun Generator.Companion.nonZeroDoubles(
    min: Double = -Double.MAX_VALUE,
    max: Double = Double.MAX_VALUE
): Generator<Double> =
 ...
 */
public fun Generator.Companion.booleans(): Generator<Boolean> = Generator { it.nextBoolean() }

/**
 * Returns a generator including [Double.NaN] in the samples
 */
public fun Generator<Double>.withNaN(): Generator<Double> = withSamples(Double.NaN)
